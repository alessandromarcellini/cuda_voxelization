...
__global__ void extract_active_voxels(int* d_voxels, Voxel* d_active_voxels, int* d_num_active_voxels) { // TODO warp divergence
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= NUM_TOT_VOXELS) return;

    int curr_voxel_final_num_points = d_voxels[idx];
    if (curr_voxel_final_num_points > MIN_POINTS_IN_VOXEL_TO_RENDER) {
        int out_idx = atomicAdd(d_num_active_voxels, 1);

        //calcolo coordinate voxel a partire da idx:
        // Calcola le coordinate x,y,z a partire dall'indice lineare
        int temp = idx;
        ...
        // aggiunta del voxel all'array di voxel in uscita
        d_active_voxels[out_idx].x = x;
        d_active_voxels[out_idx].y = y;
        d_active_voxels[out_idx].z = z;
        d_active_voxels[out_idx].num_points = curr_voxel_final_num_points;
    }
}



__global__ void setup_voxels(Voxel* voxels) { // TODO __restrict__ ??????
    ...
    // UNICA Scrittura in Memoria Globale
    voxels[idx].x = x;
    voxels[idx].y = y;
    voxels[idx].z = z;
...
// __global__ void reset_voxels(int* voxels_num_points) {
//     int idx = blockIdx.x * blockDim.x + threadIdx.x;
//     if (idx >= NUM_TOT_VOXELS) return;
...
int main(void) {
    ...
    // setup_voxels<<<gridSetupVoxels, blockSetupVoxels>>>(d_voxels_num_points_output);
    ...
    while(recv(client_fd, &num_points, sizeof(int), 0) > 0) {
        ...
        CHECK(cudaMemset(d_voxels_num_points_output, 0, NUM_TOT_VOXELS * sizeof(int)));// reset_voxels<<<gridResetVoxels, blockResetVoxels>>>(d_voxels_num_points_output);
